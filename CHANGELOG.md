...
- Added Clap detector timing and rejection diagnostics before changing live
...
- Exact-time Calendar queries no longer reuse a different event from the same
  day when composing a Workspace mail draft, and missing matches now return a
  Calendar-specific message before contact resolution.
